    ...
    if ($_POST['btn_action'] == 3) {
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                $sql = "SELECT pt.item_name, pdt.weight, pdt.melt, pdt.wast, pdt.rate, pdt.total_fine, pdt.price1, pdt.amount, pdt.lab_rate, pdt.other_charge, pdt.total_mcharge, pdt.grand_tot FROM purchasedetail_tbl AS pdt JOIN product_tbl as pt ON pt.product_id=pdt.product_id WHERE pdt.pid=" . $row['pid'];
